...
def get_result_data_from_user():
    file_name = u.get_menu_option(
        "File Name (in ./results folder): ", os.listdir(u.RESULTS_FOLDER), False)

    return pd.read_csv(f"{u.RESULTS_FOLDER}/{file_name}")


def get_hyperparameters_data_from_user():
    file_name = u.get_menu_option("File Name (in ./svm-results/data folder): ",
                                  os.listdir(f"{u.SVM_RESULTS_FOLDER}/{u.SVM_RESULTS_FOLDER_DATA}"), False)
...
def execute_ddqn_svm(env, df, pair_symbol):
    ...
    episode_rewards, profits = u.mini_batch_train(
        env, agent, MAX_EPISODES, MAX_STEPS, BATCH_SIZE)

    result_df = u.generate_rewards_profits_dataframe(episode_rewards, profits)
    result_df.to_csv(
        f"{u.RESULTS_FOLDER}/DDQN-SVM_Results_{MAX_EPISODES}_{datetime.now()}-{pair_symbol}.csv")

    print(f"\n\n== DDQN + SVC COMPLETE ==")
    print(
        f"Average Rewards: {u.list_average(episode_rewards)} | Total Profits: {sum(profits)}")
...
def execute_ddqn(env, pair_symbol):
    ...
    result_df = u.generate_rewards_profits_dataframe(episode_rewards, profits)
    result_df.to_csv(
        f"{u.RESULTS_FOLDER}/DDQN_Results_{MAX_EPISODES}_{datetime.now()}-{pair_symbol}.csv")
...
def train_svc():
    ...
    kernel_type = u.get_menu_option("Kernel Type ['linear', 'poly', 'rbf', 'sigmoid', 'precomputed']: ", [
                                    'linear', 'poly', 'rbf', 'sigmoid', 'precomputed'], False)
    pair_symbol = u.get_alphanumeric("Training Pair Symbol (e.g. BTC/IDR): ")
    gamma = u.get_int("Gamma: ")
    C = u.get_int("C: ")
    classifier = svc.SupportVectorClassifier(
        df, data_sample_count, kernel_type, pair_symbol, gamma, C)
...
def generate_model(accuracy_data, df, data_sample_count, kernel_type, pair_symbol, gamma, C, save_model_flag):
    model = svc.SupportVectorClassifier(
        df, data_sample_count, kernel_type, pair_symbol, gamma, C, save_model_flag)
    ...
    with Manager() as M:
        ...
        for gamma in range(Min, Max+1, 1):
            for C in range(Min, Max+1, 1):
                ...
                pool.apply_async(generate_model, args=(
                    accuracy_data, df, scaled_sample_count, kernel_type, pair_symbol, gamma, C, save_model_flag))
...
if opt == 1:
    start_year = u.get_int("Start Year: ")
    end_year = u.get_int("End Year: ")
    pair = u.get_menu_option(
        "Select Pair (e.g. BTC/IDR): ", ["BTC/IDR", "ETH/IDR", "USDT/IDR"], False)
    ...
    result_df.to_csv(
        f"{u.RESULTS_FOLDER}/SVM_Results_{MAX_EPISODES}_{datetime.now()}-{pair_symbol}.csv")
    ...
    plt.savefig(
        f"{u.SVM_RESULTS_FOLDER}/{u.SVM_RESULTS_FOLDER_GRAPHICS}/{file_name}")
